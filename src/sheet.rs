...
impl SheetManager {
    pub fn new(map_addr: i32) -> SheetManager {
        SheetManager {
            ...
            // sheets_data: vec![Sheet::new(); MAX_SHEETS],
        }
    }
    ...
    pub fn updown(&mut self, sheet_index: usize, oz: Option<usize>) {
        ...
        self.sheets_data[sheet_index].z = oz;
            ...
            if let Some(o) = old {
                    ...
                    if let Some(zmax) = self.z_max {
                        if zmax > o {
                            for h in o..zmax as usize {
                                self.sheets[h] = self.sheets[h + 1];
                                self.sheets_data[self.sheets[h]].z = Some(h);
                            }
                        }
                ...
                if let Some(z) = oz {
                    ...
                    for h in z..zmax {
                        ...
                        self.sheets_data[self.sheets[h + 1]].z = Some(h + 1);
                    }
                    self.sheets[z] = sheet_index;
                    if let Some(zmax) = self.z_max {
